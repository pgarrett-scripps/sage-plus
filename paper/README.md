...
## Expanded comparative report

The evaluation includes public identification yield and disagreement,
workload-dependent resources, worker scaling, entrapment calibration, synthetic
PTM acceptance, matched LFQ accuracy, and species-absent control diagnostics.
Every comparison retains the same Sage and Sage Plus released executable pair.

The September 15 report extension is frozen in
`analysis/data/report-extension/plan.json`. New execution outputs are under
`/data/sage-plus-scientific/report-extension-20260915`. They use the original
executables and selected public inputs. No build or historical release is added.
The extension is post hoc and is not a held-out validation dataset.

`analysis/scripts/collect_report.py` derives small report snapshots with source
hashes. The `gen_report_figure.py` and `gen_report_table.py` generators use those
snapshots. The original pilot evidence and its analysis scripts remain intact.
