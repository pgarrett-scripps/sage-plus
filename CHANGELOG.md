...
- PTM site localization (`ptm_localization` setting / `--localize` flag). After spectrum FDR assignment, each passing target PSM carrying a variable modification is re-scored across candidate sites using site-determining ions, producing an AScore-style delta and per-site localization probabilities. Keeping localization out of the main scoring hot path avoids work on rejected PSMs. Two site reports are written in the selected TSV or Parquet format: `results.sage.ptm-sites.*` and `results.sage.protein-sites.*`. `ptm_localization.psm_q_value` controls the spectrum identification q-value cutoff (default: 0.01); it is separate from localization confidence.
