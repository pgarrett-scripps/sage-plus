...
- PTM site localization (`localize` setting / `--localize` flag). For each FDR-passing PSM carrying a variable modification, sage redistributes the modification across all candidate residues, re-scores each arrangement against the spectrum using site-determining ions, and reports an AScore-style delta plus per-site localization probabilities. Two new TSV reports are written: `results.sage.ptm-sites.tsv` (one row per localized PSM site) and `results.sage.protein-sites.tsv` (best localization per protein/peptide site, aggregated across PSMs). `localize_q_value` controls the spectrum q-value cutoff for inclusion (default: 0.01).
