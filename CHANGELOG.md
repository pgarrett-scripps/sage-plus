...
- PTM site localization (`ptm_localization` setting / `--localize` flag). For each FDR-passing PSM carrying a variable modification, sage redistributes the modification across all candidate residues, re-scores each arrangement against the spectrum using site-determining ions, and reports an AScore-style delta plus per-site localization probabilities. Two new site reports are written in the selected TSV or Parquet format: `results.sage.ptm-sites.*` (one row per localized PSM site) and `results.sage.protein-sites.*` (best localization per protein/peptide site, aggregated across PSMs). `ptm_localization.psm_q_value` controls the spectrum identification q-value cutoff for inclusion (default: 0.01); it is separate from localization confidence.
