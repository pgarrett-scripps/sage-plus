...
impl CloudObject {
    pub(crate) fn new(url: impl AsRef<str>) -> Result<Self, CloudError> {
        let (store, path, base) = CloudProvider::authenticate_store(url.as_ref())?;
        Ok(CloudObject { store, path, base })
    }

    #[allow(clippy::len_without_is_empty)]
    pub(crate) fn len(&self) -> Result<usize, CloudError> {
        let size = block_on(self.store.head(&self.path))
            .map_err(|e| CloudError::ThirdParty(Box::new(e)))?
            .size;
        usize::try_from(size).map_err(|e| CloudError::ThirdParty(Box::new(e)))
    }

    pub(crate) fn children(&self) -> Result<Vec<String>, CloudError> {
        let mut children = vec![];
        let prefix = Some(self.path.clone());
        let list_result = block_on(self.store.list_with_delimiter(prefix.as_ref()))
            .map_err(|e| CloudError::ThirdParty(Box::new(e)))?;
    ...
    pub(crate) fn download_to(&self, dest: &std::path::Path) -> Result<(), CloudError> {
        ...
        block_on(async {
            ...
            while offset < expected_size {
                ...
                let bytes = self
                    .store
                    .get_range(&self.path, offset as u64..end as u64)
                    .await
                    .map_err(|e| CloudError::ThirdParty(Box::new(e)))?;
                if bytes.len() != end - offset {
                    return Err(CloudError::ThirdParty(Box::new(std::io::Error::new(
    ...
    pub(crate) fn upload_bytes(&self, bytes: Vec<u8>) -> Result<(), CloudError> {
        block_on(async move {
            self.store
    ...
    pub(crate) fn upload_from(&self, src: impl AsRef<std::path::Path>) -> Result<(), CloudError> {
        let src = src.as_ref();
        block_on(async {
            ...
            let mut writer =
                object_store::buffered::BufWriter::new(self.store.clone(), self.path.clone());
