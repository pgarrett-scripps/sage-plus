...
#[test]
fn motif_sites_restrict_variable_modifications_and_mirror_decoys() {
    use crate::database::Builder;
    ...
    .unwrap();
    // Canonical spelling is required, like other explicit site names.
    assert!(serde_json::from_value::<Builder>(serde_json::json!({
        "variable_mods": {"HexNAc": {"mass": 203.0, "sites": ["motif:N*-{P}-[TS]"]}}
    }))
    .is_err());
    assert!(serde_json::from_value::<Builder>(serde_json::json!({
    ...
    let fasta = Fasta::parse(">P1\nMAGNGTKANPSKAANKSLLRGASPK\n".into(), "rev_", true).unwrap();
    let peptides = parameters.modify_digests(parameters.digest_unmodified(&fasta));
    let rendered = peptides
        .iter()
        .map(|peptide| (peptide.decoy, peptide.to_string()))
        .collect::<std::collections::HashSet<_>>();

    // In-peptide sequon, and a sequon completed by the flanking protein residue.
    assert!(
        rendered.contains(&(false, "MAGN[HexNAc]GTK".into())),
        "{rendered:?}"
    );
    assert!(
        rendered.contains(&(false, "AAN[HexNAc]K".into())),
        "{rendered:?}"
    );
    // N-P-S is not a sequon.
    assert!(!rendered
        .iter()
        .any(|(decoy, p)| !decoy && p.starts_with("AN[")));
    // R-x-x-S completed by the preceding cleavage-site arginine.
    assert!(
        rendered.contains(&(false, "GAS[Phospho]PK".into())),
        "{rendered:?}"
    );
    assert!(!rendered
        .iter()
        .any(|(decoy, p)| !decoy && p.starts_with("S[")));

    // The generated decoy AN[+]AK mirrors the target placement. Localization
    // must see the mirrored target motif, not the literal decoy sequence.
    let decoy = peptides
        .iter()
        .find(|peptide| peptide.decoy && peptide.to_string() == "AN[HexNAc]AK")
        .unwrap();
    let motif: ModificationSpecificity = "motif:N*-{P}-[ST]".parse().unwrap();
    assert_eq!(decoy.rule_sites(motif, true), vec![Site::Sequence(1)]);
    assert!(decoy.rule_sites(motif, false).is_empty());
