...
- **mass_recalibration**: `"off"` (default), `"static"`, `"linear"`, or `"auto"`. Correct precursor and fragment m/z during the search. Each file is first searched on a sample of up to 25,000 MS2 spectra with a precursor, stratified by acquisition group: each group keeps its share of the sample (at least one spectrum), taken at even positions across its own spectra, so interleaved scan cycles cannot be sampled unevenly; rank-1 target PSMs at 1% Poisson spectrum q-value supply signed precursor errors (isotope error 0, no mass offset) and per-PSM median signed fragment errors. Q-values are computed within each acquisition group, so a low-accuracy group searched with a wide fragment window cannot swamp an accurate group with decoys; the precursor model is fitted on the union of every group's confident PSMs and each fragment model on its own group's. Every PSM is assigned to a fit or validation set (30%) by a stable hash of its spectrum ID. Each file gets one precursor model and one fragment model per acquisition group, where a group is the MS2 mass analyzer (Orbitrap, Astral, TOF, ion trap) and activation (HCD, CID, ETD, EThcD, ETciD) of the spectrum. Groups are read from the Thermo filter string (mzML `MS:1000512`, mzMLb, Thermo RAW), otherwise from mzML instrument configurations (scan `instrumentConfigurationRef` or the run default; the Astral analyzer is `MS:1003379`) and precursor activation terms; Bruker TDF is TOF/CID. Thermo RAW files whose OpenTFRaw scan events are out of step with their trailers (some Orbitrap Fusion files) have no trustworthy filter, so all their scans are `unknown/unknown` and a warning says so; convert such files to mzML for per-analyzer models. An unknown analyzer is not treated as low accuracy, so the `unknown/unknown` group is fitted and validated like any other. Resolution and AGC target are not part of the key: they are rarely recorded per scan and would split files too thinly. Each group's fragment model is selected independently with the same gates below, so an ion-trap CID group and an Orbitrap HCD group in the same file never share a pooled correction. Ion-trap groups are never corrected (`skipped: "low_accuracy_analyzer"`), groups with too few PSMs keep no correction (`"too_few_psms"`), and spectra whose group has no model are left uncorrected. The precursor model stays per file because precursor m/z comes from the survey scan. The mode sets the most flexible model allowed:
  ...
  A more flexible model is accepted only when it lowers the validation median absolute residual (2% for static, 5% for each further step), no retention-time or m/z tercile gets more than 2% worse, and there are enough PSMs (50 static, 200 linear, 1,000 smooth with 50 per interval) spread over at least 10 minutes and 100 m/z (200 for fragments). Otherwise the next simpler model, or no correction, is kept. The accepted form is refit on all inlier PSMs, is flat outside the fitted range, and is capped at the tolerance half-width (fragment Da tolerances are converted at m/z 2000). A Da precursor tolerance disables the precursor model. The full search then reruns with corrected masses on targets and decoys alike, using the configured tolerances; the preliminary prefilter is widened by one tolerance half-width on each side so corrected windows remain covered. Wide-window searches ignore this setting. Chosen models, candidate validation scores, and residual bins are written per file (fragment: one entry per acquisition group, with `analyzer`, `activation`, and `spectra`) to `run-summary.json` under `models.mass_recalibration`, and files corrected at search time skip the post-search mass-error feature alignment.
- **tolerance_mode**: `"fixed"` (default) or `"auto"`. With `auto`, the discovery pass described under `mass_recalibration` runs even when `mass_recalibration` is `"off"`, and its residuals narrow the search tolerances: the precursor tolerance per file, and the fragment tolerance per file and acquisition group. Residuals are taken after the model chosen on the fit set (raw errors when no model is chosen). They are fitted as a mixture `pi·signal + (1 - pi)·uniform` over the configured window by EM (median/MAD start, at most 500 iterations), because confident PSMs include random matches that fill whatever window is searched, so a percentile of all residuals cannot shrink it. Three signal forms are fitted on the fit set: a Gaussian, a Student-t with 4 degrees of freedom, and two Gaussians with a shared center. A heavier form is chosen only if its held-out log-likelihood is at least 0.005 nats per point better. The window is `center ± max(h, floor)`, where `h` holds 99% of the signal; the floor is 3 ppm for precursors and 5 ppm for fragments. Precursor PSMs matched at a non-zero isotope error have a lower signal fraction and a wider, shifted signal, so when there are at least 200 of them they are fitted as their own subgroup (fewer are left out), and the window is the union of both signal windows, because every isotope hypothesis is searched with it. They never fit models. The window is clipped to the configured tolerance on each side, so `auto` only narrows, and it is applied only if it holds at least 98% of the estimated held-out signal (the signal responsibilities of held-out residuals inside it over all of them). A fit is not trusted, and the configured tolerance is kept, when EM did not converge, the scale is at a bound, the signal fraction is below 0.2, or `h` exceeds 90% of the configured half-width. The configured tolerance is also kept for Da or percent tolerances, fewer than 100 held-out PSMs, low held-out coverage, ion-trap groups, a group without a residual estimate, and, for the precursor, when mass offsets are configured or either side of a ppm window exceeds 100 ppm (open search). Targets and decoys use the same windows. The candidate prefilter keeps the configured (or recalibration-widened) windows, and the discriminant model's mass features use the configured tolerance. `run-summary.json` records, per file, `precursor_tolerance` and, per acquisition group, `tolerance`: the configured and applied windows, whether it was narrowed or why not (`skipped`), the center and 99% half-width, held-out PSM and point counts, isotope-error points, `validation_coverage`, and `signal` (and `isotope_error_signal`): the chosen form, signal fraction, center, scale, 99% half-width, iterations, convergence, and the held-out log-likelihood of each candidate form. On the benchmark data `auto` did not beat a well-chosen fixed tolerance: from ±10/20 ppm it lost about 4% of PSMs, and from ±50 ppm it recovered about the IDs of ±10/20 ppm fixed; see `MASS_CALIBRATION_DESIGN.md`. Wide-window searches ignore this setting.
