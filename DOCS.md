...
- **tolerance_mode**: `"fixed"` (default) or `"auto"`. With `auto`, the discovery pass described under `mass_recalibration` runs even when `mass_recalibration` is `"off"`, and its residuals narrow the search tolerances: the precursor tolerance per file, and the fragment tolerance per file and acquisition group. Residuals are taken after the model chosen on the fit set (raw errors when no model is chosen). The window is `center ± max(1.2·q99, floor)`, where `center` is the fit-set residual median and `q99` the 99th percentile of absolute fit-set residuals around it (all points, no outlier cut); the floor is 3 ppm for precursors and 5 ppm for fragments. A percentile is used because mass errors of confident PSMs are heavy-tailed: on the benchmark data four robust standard deviations (`4·1.4826·MAD`) covered only about 95% of them and lost 2-9% of PSMs. The window is clipped to the configured tolerance on each side, so `auto` only narrows, and it is applied only if it covers at least 98% of held-out residuals. The configured tolerance is kept for Da or percent tolerances, fewer than 100 held-out PSMs, low held-out coverage, ion-trap groups, a group without a residual estimate, and, for the precursor, when mass offsets are configured or either side of a ppm window exceeds 100 ppm (open search). Isotope errors are searched as before, each with the narrowed window; because of that, precursor PSMs matched at a non-zero isotope error (whose errors are wider) are added to the residuals that size the precursor window, though they never fit models. Targets and decoys use the same windows. The candidate prefilter keeps the configured (or recalibration-widened) windows, and the discriminant model's mass features use the configured tolerance. `run-summary.json` records, per file, `precursor_tolerance` and, per acquisition group, `tolerance`: the configured and applied windows, whether it was narrowed or why not, the center, half-width, fit-set and held-out 99th percentiles and robust sigmas, held-out PSM and point counts, isotope-error points, and `validation_coverage`, the fraction of held-out residuals inside the proposed window (every held-out residual lies inside the configured window). Wide-window searches ignore this setting.
